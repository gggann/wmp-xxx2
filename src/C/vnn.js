...
export default Vue.component('vnn', {
    ...
    <div id="부스팅점수" v-for="(v,i) in boostCategories" :key="i">
        {{setalljson(v.categoryCd)+" "}}<span style="color:red">{{v.score}}</span>
    </div>
    <div id="카테통계특가">
        <div id="부스팅점수" v-for="(v,i) in boostCategories1" :key="i">{{setalljson(v.categoryCd)+" "}}<span
                style="color:blue">{{v.score}}</span>
        </div>
        <!-- @ -->
        <div id="카테통계특가">

            <div id="카테상세">
                <div v-for="(v,i) in getWordCnt(cate통계)" :key="i">{{i+" >  "+v}}
                </div>
            </div>
            <div id="카테코드">
                <div v-for="(v,i) in getWordCnt(cate통계코드)" :key="i"><span
                        @click="Commandid(parseInt(i.slice(0,8)))">{{i.slice(0,8)}}</span><span
                        @click="InputCateCd=i.slice(10,17);radio='카테고리추출'">{{i.slice(10,33)+" >  "+v}}</span>
                </div>
            </div>
    ...
    <label for="nosave">nosave</label>-->
            ...
            <button id="pricebutton1" @click="upprice"><span v-if="uppriceif">가격↓</span><span
            ...
            <vodal :modalstorename="dealliststorename" :modalhtml="deallisthtml" :modalimg="deallistimg"
            ...
            <button id="remove_opacity" @click="cate_all">카오</button>
            <button id="idcate_filter" @click="idcate_filter">필터</button>
        </div>
        ...
        <div>
            ...
            <!-- <strong v-if="force || errata">네이버 오타<span style="color:red">은{{force}}{{errata}}</span>로 검색 </strong> -->
        </div>

        <div id="dealtype" :class="v.link.type + ' ' + v.catalog.lowestPriceProductType + ' ' + v.link.value" v-for="(v,i) in info1" :key="v.link.value">
            ...
            <a :href='checkimgsrc(v)' target="_blank"><img class="card-img-top" :src=cimg(v) alt="Card image cap"
                    :style="'opacity:'+v.link.check"></a>


            <div id="lastdeal" v-if="i==info1.length-1">
            </div>
            <div id="정보">
                <div id="기본정보">
                    ...
                    <button style="font-size:10px;"
                        @click='postmanctlg(v)'>cms</button>
                    ...
                    <div id="상세정보">
                        ...
                        <div id="내부키워드" v-html="highlightData(v.administrator.product.innerKeyword)">
                        </div>
                        <div id="딜내부키워드" v-html="highlightData(v.administrator.product.dealInnerKeyword)">
                        </div>
                        <div id="파트너정보"
    ...
    <!-- @@@@@@@@@@@@@@@ -->
    <div id="checkarea">
    <div id="dealtype" :class="v.link.type + ' ' + v.link.value" v-for="(v,i) in info2" :key="v.link.value">
            ...
            <div id="lastdeal" v-if="i==info2.length-1">
            </div>
            <div id="정보">
                <div id="기본정보">
   ...
   `,
    data() {
        return {
            ...
            금지어:['유아','어린이','키즈','주니어','남아','여아','강아지','애견','애완','반려','동물','고양이','냥이'],
            제외아이디수:'',
            //dealorone:true
        }
    },
    ...
    methods: {
        async idcate_filter() {
         ...
         async cate_all(){
             console.log('카테디테일_all')
           for (let i of this.info){
             if(i.catalog.lowestPriceProductType!=='EP'){
                 this.cate_ID(i)
             }
           }
         },
         async cate_ID(vv){
             ...
             .then(res => {
                 ...
                 if(vv.administrator.product.physicsCategoryIds.lcateNm!=checkdealtype.basic.lcateNm){
                     if (document.querySelector("#dealname").value == "") {
                         document.querySelector("#dealname").value = vv.administrator.product.catalogNo;
                     } else {
                         document.querySelector("#dealname").value = document.querySelector("#dealname").value + "\r\n" + (vv.administrator.product.catalogNo).trim()
                     }
                 }       
             })
             .catch(function (error) { // 请求失败处理
                 console.log(vv.link.value+error);
             });
         },
        cimg(v){
            // if(v.originImgUrl){
       ...
       $("#dealtype > a > img").css("opacity","1")
        ...
        checkimgsrc(v){
            ...
            }else  if(v.catalog?.lowestPriceProductType==="WMP" && v.mediumImgUrl){
                let prodid = v.lowestProd.value
                //return "https://front.wemakeprice.com/" + this.stype(v.link.type)+"/"+v.link.value   //以后复原这个 删除上下两个
                return "https://front.wemakeprice.com/" + this.stype(v.link.type)+"/" + prodid
            }
        },
        checkctlglength(v) {
        ...
        async deallist2(vv) {
            ...
            await axios
                ...
                .catch(function (error) { // 请求失败处理
                 ...
                 await axios
               // .get('/product/' + vv.link.value)
               .get('/product/' + vv.lowestProd.value)
                .then(res => {
    ...
    computed: {
        idcate: async function () {
            ...
            //  return axios.get('http://localhost/wmpjs/savejson/'+encodeURIComponent(e.trim().replace(/ /g, ''))+'_c1.json') 
                }
                function o(e) {
                 return axios.get(t + encodeURIComponent(e) + "&os=android&page=2" + a)
               //return axios.get('http://localhost/wmpjs/savejson/'+encodeURIComponent(e.trim().replace(/ /g, ''))+'_c2.json') 
                }
                function n(e) {
               return axios.get(t + encodeURIComponent(e) + "&os=android&page=3" + a)
               // return axios.get('http://localhost/wmpjs/savejson/'+encodeURIComponent(e.trim().replace(/ /g, ''))+'_c3.json') 
                }
