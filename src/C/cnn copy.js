...
export default Vue.component('cnn', {
    ...
    <div>
        ...
        <div>
            ...
            <div id="dealtype" :class="dealtype(v.link.type) + ' ' + v.salePriceSuffix + ' ' + v.link.value" v-for="(v,i) in info1" :key="v.link.value">
                    ...
                    <span class="index" @click="out_info1(v)">{{dealindex[i]}}##{{v.administrator.product.brandKeyword}}</span> <span id="브랜드" @click="handrank(v)"
        ...
        <div id="dealtype" :class="dealtype(v.link.type) + ' ' + v.salePriceSuffix + ' ' + v.link.value" v-for="(v,i) in info2" :key="v.link.value">
                ...
                <span class="index" @click="out_info2(v)">{{dealindex[i]}}##{{v.administrator.product.brandKeyword}}</span> <span id="브랜드" @click="handrank(v)"
                    v-html="highlightData(v.administrator.product.brandKeyword)"></span>
                <a :href="checkimgsrc(v)" target="_blank"><img class="card-img-top" :src=cimg(v)
                        alt="Card image cap" :style="'opacity:'+v.link.check"></a>
   ...
   `,
    ...
    methods: {
        ...
        async idcate_filter() {
            this.deallisthtml = ""
             await  axios.get(wsa_urlf1 + encodeURIComponent(this.message) + "&os=android&page=1" + wsa_urlf2).then(res=>{
        ...
        idcate: async function () {
            if (this.info = [],
                ...
                async function nnn(k) {
                    return axios.get(wsa_url1 + k + "&os=android&page=1" + wsa_url2)
                }
                async function nn(k) {
                        return axios.get(wsa_url1 + k + "&os=android&page=2" + wsa_url2)
                }
                async function  n(k) {
                        return axios.get(wsa_url1 + k + "&os=android&page=3" + wsa_url2)
                }
