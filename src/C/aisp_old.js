...
export default Vue.component('aisp', {
  ...
  <div id="dealtype" :class="v.link.type + ' '+v.link.value" v-for="(v, i) in aiplus" :key="v.link.type">
      <span class="index">{{dealindexaiplus[i]
    }}</span>
        ...
        " " + v.link.type
          ...
          <div id="카테고리">
              <span @dblclick="Commandtext(v.categoryInfo.lcateCode)"
                  @click="Commandtext(v.categoryInfo.lcateName),check_cate(v.categoryInfo.lcateCode)">{{ v.categoryInfo.lcateName }}</span>
              >
              <span @dblclick="Commandtext(v.categoryInfo.mcateCode)"
                  @click="Commandtext(v.categoryInfo.mcateName),check_cate(v.categoryInfo.mcateCode)">{{ v.categoryInfo.mcateName }}</span>
              >
          </div>
      </div>
  </div>
    ...
    async aicate() {
      ...
      if (this.checkif) {
        ...
        await axios
          ...
          //  if (res.data.data.adAiPlus.deals) {
