...
export default Vue.component('ann', {
    ...
    <aisp :highlightData="highlightData" :message="message" :deallist3="deallist3" :deallist2="deallist2" :대표키워드="대표키워드" @ai_total= son_total>
    </aisp>
   



    <div id="총디브" v-on="idcate">
    </div>
    ...
    <div id="카테통계특가">
        ...
        <div id="dealtype" :class="v.link.type + ' ' + v.link.value" v-for="(v,i) in info1" :key="v.link.value">
            ...
            <div id="정보">
                <div id="기본정보">
                    <div
                        v-if="v.link.type=='DEAL' || v.link.type=='PROD' || v.link.type=='DEAL cms' || v.link.type=='PROD cms'">
   ...
   `,
    ...
    methods: {
        ...
        //     var aaaa = ""
        //     await axios
        //         .get('/deal/' + vv.link.value,{ cache: true })
        //         .then(res => {
                    ...
                    await axios
                        ...
                        .then(res => {
                            if (page == 1) {
                                ...
                                that.제외아이디수 = res.data.data.keywordRanking.excludedProductIds.length;
                                that.an = new Map();
    ...
    computed: {
        idcate: async function () {
            ...
            if (this.checkif) {
                ...
                for (let page = 1; page < this.page_count; page++) {
                    await axios
                   .get(urlmain + encodeURIComponent(that.message) + "&os=android&page=" + page + '&perPage=30&period=monthly&price=&review=&searchDest=TAB_SPECIAL&selectTab=special&sort=weight&testScript=&testTarget=&ueKeywordInfo=Y')
                  //.get('http://localhost/wmpjs/savejson/'+encodeURIComponent(that.message.trim().replace(/ /g, ''))+'_s'+page+'.json')
                        ...
                        .then(res => {
                            ...
                            console.log(this.ai_count)
                            if(this.ai_count>=60) {this.page_count = 2}
                            else if(60>this.ai_count>30){this.page_count=3}else if(30>this.ai_count>30){this.page_count=3}
                        })
                        .catch(function (error) { // 请求失败处理
                            console.log(error);
                        });
                }
                this.info = this.info.slice(0,90-this.ai_count)
        ...
        info1() {
            if (this.radio == "디폴트") {
                ...
                return this.info.filter((p) => {
                    return (this.dealvalue.indexOf(p.link.value) != -1) || (p.link.type == "PROD" && p.dispNm.substring(0, this.localkwdlength).toLowerCase().indexOf(this.keyword.toLowerCase()) == -1)
                })
