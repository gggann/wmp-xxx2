...
export default Vue.component('cnnb', {
    ...
    <div>
        ...
        <div id="카테통계특가">
            ...
            <div>
                ...
                <div>
                    ...
                    <button @click="hidecatalog"><span v-if="catalogif">카탈숨기</span><span v-else>카탈만보기</span></button>
        ...
        <div>
            ...
            <div id="dealtype" :class="v.link.type + ' ' + v.link.value" v-for="(v,i) in info1" :key="v.link.value">
                    ...
                    <a :href="checkimgsrc(v)" target="_blank"><img class="card-img-top" :src=cimg(v)
   ...
   `,
    ...
    methods: {
        ...
        //                 let baseData = canvas.toDataURL('image/png', .8)
        ...
        reload() {
                ...
                let showtime=()=>{
                    tim--;
                    this.timer = tim;
                    document.querySelector('title').textContent = tim + ' 결과 가다리는중';
                    if(tim==0){clearInterval(timer);console.log('over');location.reload();document.querySelector('title').textContent = ' 보시오';}
                }
            ...
            if (localStorage.getItem('cms_login_id') == 'F000000256') {
                ...
                for (let i = 0; i < this.info1.length; i++) {
                    if (this.info1[i].administrator.product.catalogNo) {
                        let id = this.info1[i].administrator.product.catalogNo
                        createdRows.push({
        ...
        async postmanctlg(v) {
            ...
            let poststr = 'createdRows=[{"rep_keyword":"' + encodeURIComponent(this.대표키워드) + '","keywords":"","wst_cd":"WST000","type_cd":"NKT004","ban_keyword":' + id + ',"comment":"' + this.CmsoptionValue + '","use_yn":"Y","rowKey":1}]'
            axios.post('/adplus2/ban/keyword/save',
                poststr,
            ).then(res => {
        ...
        async postman(id, dealname) {
         ...
         let poststr = 'createdRows=[{"rep_keyword":"' + encodeURIComponent(this.대표키워드) + '","keywords":"","wst_cd":"WST000","type_cd":"NKT002","ban_keyword":' + id + ',"comment":"' + this.CmsoptionValue + '","use_yn":"Y","rowKey":1}]'
          ...
          if (this.대표키워드 !== '') {
            axios.post('/adplus2/ban/keyword/save',poststr).then(res => {
                console.log(poststr)
        ...
        async alldeal() {
            var that = this
            async function an() {
                for (let i of that.info1)
                if (i.link.type.includes('DEAL')) {
                        that.deallistall(i)
                    }
            }
        //    if (localStorage.getItem('cms_login_id') == 'F000000256') {await an()}
             ////
            this.checklisttext1 = "🟢"
            this.radio = "ck1"
        },
        ...
        idcate2: async function () {
            if (this.checkif) {
                ...
                let count = Number(this.page_count)
                    ...
                    await axios
                        .get(wsa_url1_b + encodeURIComponent(that.message) + "&os=android&page=" + page + wsa_url2)
                        .then(res => {
                            if (page == 1) {
    ...
    computed: {
        ...
        idcate: async function () {
            if (this.info = [],
                ...
                let k = encodeURIComponent(this.message)
                async function nnn(k) {
                    return axios.get(wsa_url1_b + k + "&os=android&page=1" + wsa_url2)
                }
                async function nn(k) {
                        return axios.get(wsa_url1_b + k + "&os=android&page=2" + wsa_url2)
                }
                async function  n(k) {
                        return axios.get(wsa_url1_b + k + "&os=android&page=3" + wsa_url2)
                }
                
                await axios.all([n(k), nn(k), nnn(k)]).then(axios.spread((function (q, a, t) {
                        ...
                        e.info = [...e.info, ...q.data.data.deals]
                }
                )));
        ...
        info1() {
            if (this.radio == "디폴트") {
                ...
                return this.info.filter((p) => {
                    return (p.administrator.product.partnerName !==null && (p.administrator.product.partnerName?.toLowerCase()).indexOf(this.keyword.toLowerCase()) > -1)  && p.dispNm !== '@@@'
                })
                ...
                return this.info.filter((p) => {
                  //  return (this.dealvalue.indexOf(p.link.value) != -1) || (p.link.type !== "DEAL" && p.dispNm.substring(0, this.localkwdlength).toLowerCase().indexOf(this.keyword.toLowerCase()) == -1)
                    return (p.dispNm.substring(0, this.localkwdlength).toLowerCase().indexOf(this.keyword.toLowerCase()) == -1) && p.dispNm !== '@@@'
                })
