...
export default Vue.component('cnn_local', {
            ...
            <strong v-if="force || errata">네이버 오타<span style="color:red"> {{force}}{{errata}}</span>로 검색 </strong>
                ...
                <div id="dealtype" :class="v.link.type + ' ' + v.link.value + ' 삭제예정id'" v-for="(v,i) in info1" :key="v.link.value">
                    ...
                    <span class="index" @click="out_info1(v)">{{dealindex[i]}}</span> <span id="브랜드"
                        ...
                        <a :href="checkimgsrc(v)" target="_blank"><img class="card-img-top" :src=save_img(v) :onerror="defaultImg(v.mediumImgUrl)"
   ...
   `,
    ...
    methods: {
        defaultImg(v){
            return 'this.onerror=null;this.src='+'"'+v+'"'
        ...
        async navercate() {
            ...
            async deallistall(vv) {
                ...
                if(vv.list){
                    for (let i of vv.list) {
                        if (i.prodNm?.indexOf(this.kw1.toLowerCase()) !== -1) {
            ...
            async deallistall2(vv) {
                var that = this
                for (let i of vv.list) {
                    if (i.prodNm?.indexOf(this.kw1.toLowerCase()) > -1 && i.prodNm?.indexOf(this.kw2.toLowerCase()) > -1) {
                        vv.dealvalue = 1
                        return
                    } else {
                        vv.dealvalue = 0
        ...
        idcate: async function () {
            if (this.info = [],
                ...
                for (let r = 0; r < e.info.length; r++){
                    this.info[r].dispNm = this.info[r].dispNm.replace('[롯데백화점]','').replace('(현대백화점)','').replace('[현대백화점]','').replace('(현대Hmall)','').replace('[하프클럽]','').replace('[보리보리]','').replace('[선물하기]','')
